...
#Class that implementeds all the root chains on our network
class RootChain: 
    ...
    def check_new_transaction(self, transaction):
        if ( len(transaction.keys()) == 2 and "weights" in transaction.keys() and "model_arch" in transaction.keys() ):
            return True
        return False
